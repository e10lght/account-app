...
export const GraphPieChart = () => {
    ...
    const fetchData = async () => {
        ...
        const categoryNameList = categories.map(category => category.income_category_name);
    ...
    return (
        <Card style={{ margin: 10 }}>
            ...
            <CardContent>
                {monthlyIncome.categoryNameList && monthlyIncome.categoryNameList.map((value, index) => {
                    const total = Number(monthlyIncome.result[index]).toLocaleString()
                    return (
                        <Typography>{value}：{total}円</Typography>
                    )
                })}
            </CardContent>
        </Card>
