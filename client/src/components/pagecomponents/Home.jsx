import { Button, Card, CardContent, Divider, Grid, List, MenuItem, TextField, Typography } from '@mui/material'
...
export const Home = () => {
    ...
    const onClickLastmonth = () => {
        // 前月のデータ取得してグラフを再描画する
        const lastmonth = dayjs(spendMonth).tz().subtract(1, 'month').format('YYYY-MM-DD');
        setSpendMonth(lastmonth)
        actionSpend(lastmonth);

    }
    const onClickNextMonth = () => {
        // 翌月のデータ取得してグラフを再描画する
        const nextmonth = dayjs(spendMonth).tz().add(1, 'month').format('YYYY-MM-DD');
        setSpendMonth(nextmonth)
        actionSpend(nextmonth);
    }
    ...
    return (
        <>
            <Card sx={{ minWidth: '90%' }} style={{ margin: 10 }}>
                <CardContent>
                    <Typography variant="h3" sx={{ fontSize: 24 }} color="text.seconday" textAlign='center'>
                    ...
                    <Typography>
                        収入額合計：{Number(totalIncomeMonthly).toLocaleString()}円<small>※1月前の収入が対象</small>
                    </Typography>
                    <Typography>
                        支出額合計：{Number(totalSpendMonthly).toLocaleString()}円
                    </Typography>
                    <Typography>
                        {dayjs().format('M月DD日')}時点
                    </Typography>
                </CardContent>
            </Card>
            <Card style={{ margin: 10 }}>
                ...
                <Grid container justifyContent="space-around">
                    <Button variant="text" onClick={onClickLastmonth}>前月</Button>
                    <Divider />
                    {dayjs(spendMonth).tz() <= dayjs().tz().startOf('month') && <Button variant="text" onClick={onClickNextMonth}>翌月</Button>}
                </Grid>
                <Grid container justifyContent="center">
                    {graphFlag === 1 && <Grid item sm={6}><GraphVerticalChart /></Grid>}
                    {graphFlag === 1 && <Grid item sm={6}><GraphPieChart /></Grid>}
                    {graphFlag === 0 && <Grid item sm={6}><SpendVerticalChart /></Grid>}
                    {graphFlag === 0 && <Grid item sm={6}><SpendPieChart dataList={spendDatalistMonth} /></Grid>}
                </Grid>
            </Card>
        </>
