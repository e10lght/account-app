...
export const SpendPieChart = (props) => {
    ...
    const { spendMonthly = {} } = useSelector((state) => state.spendMonthly) || {};
    ...
    const fetchData = async () => {
        ...
        const categoryNameList = categories.map(category => category.spending_category_name);
        ...
        datasets: [
            {
                ...
                hoverBackgroundColor: ["#0e5a88cc", "#c9daebcc", "#007ac4cc", "#85b4d0cc"]
            }
        ]
    };
    ...
    return (
        <Card style={{ margin: 10 }}>
            ...
            <CardContent>
                {/* index使わないとだめか？ */}
                {monthlySpend.categoryNameList && monthlySpend.categoryNameList.map((value, index) => {
                    const total = Number(monthlySpend.result[index]).toLocaleString()

                    return (<Typography key={value}>{value}：{total}円</Typography>)
                })}
            </CardContent>
        </Card>
    )
